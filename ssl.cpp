   ...
   int verbose)
{
  ...
  double *C = Data->C;
  int n  = Data->n;
  double lambda_l = Options->lambda_l;
...
{ 
  ...
  while(iter<MFNITERMAX)
	...
	o_bar[i]=o[i];
      cout << " " ;
      opt=CGLS(Data,Options,ActiveSubset,Weights_bar,Outputs_bar, verbose);
        ...
        int verbose)
{
  ...
  init_vec_double(Outputs_Labeled,Data->l,0.0);
  if (verbose > 0)
    cout << "Initializing weights, unknown labels" << endl; 
  GetLabeledData(Data_Labeled,Data); /* gets labeled data and sets C=1/l */
  L2_SVM_MFN(Data_Labeled, Options, Weights,Outputs_Labeled,0, verbose);
  //Clear(Data_Labeled);
  clear_data(Data_Labeled);
  ...
  for(int i=0;i<Data->m;i++)
    Outputs->vec[i]=0.0;
  L2_SVM_MFN(Data,Options,Weights,Outputs,0, verbose); 
  int num_switches=0;
  int s=0;
  int last_round=0;
  ...
  if (verbose > 0)
  {
	...
	L2_SVM_MFN(Data,Options,Weights,Outputs,1, verbose); 
      }
  ...
  double F = transductive_cost(norm_square(Weights),Data->Y,Outputs->vec,Outputs->d,Options->lambda_l,Options->lambda_u);
  if (verbose > 0)
     ...
     int verbose)
{
  ...
  optimize_w(Data,p,Options,Weights,Outputs,0, verbose);  
  F = transductive_cost(norm_square(Weights),Data->Y,Outputs->vec,Outputs->d,Options->lambda_l,Options->lambda_u);
  F_min = F;
  ...
  while((iter1 < DA_OUTER_ITERMAX) && (H > Options->epsilon))
    ...
    if (verbose > 0)
      cout << "Optimizing p. " ;
	  optimize_p(g,Data->u,T,Options->R,p, verbose);
	  kl_divergence=KL(p,q,Data->u);
    
    if (verbose > 0)
	     cout << "Optimizing weights" << endl;
	  optimize_w(Data,p,Options,Weights,Outputs,1, verbose);
	  F = transductive_cost(norm_square(Weights),Data->Y,Outputs->vec,Outputs->d,Options->lambda_l,Options->lambda_u);
	  if(F < F_min)
	    {
	      F_min = F;
...
{ 
  ...
  while(iter<MFNITERMAX)
      ...
      if(opt && opt2) // l
	{
      ...
      delta=line_search(w,w_bar,lambda_l,o,o_bar,Y,C,n,m+u); 
      if (verbose > 0)
        cout << " LINE_SEARCH delta = " << delta << endl;     
      F_old=F;
      F=0.0;
...
double transductive_cost(double normWeights,double *Y, double *Outputs, int m, double lambda_l,double lambda_u)
{
  ...
  double F;
  F = 0.5*(lambda_l*normWeights + lambda_u*F1/u + F2/l);
  return F;
}
         ...
         int verbose)
{
  ...
  switch(Options->algo)
    ...
    case -1:
      ...
      if (verbose > 0)
        printf("Regularized Least Squares Regression (CGLS)\n");
      optimality=CGLS(Data,Options,Subset,Weights,Outputs, verbose);
      break;
    case RLS:
      //cout << "Regularized Least Squares Classification (CGLS)\n" << endl;
      if (verbose > 0)
        printf("Regularized Least Squares Classification (CGLS)\n");
      optimality=CGLS(Data,Options,Subset,Weights,Outputs, verbose);
      break;
    case SVM:
      //cout << "Modified Finite Newton L2-SVM (L2-SVM-MFN)\n" << endl;
      if (verbose > 0)
        printf("Modified Finite Newton L2-SVM (L2-SVM-MFN)\n");
      optimality=L2_SVM_MFN(Data,Options,Weights,Outputs,0, verbose);
      break;
    case TSVM:
      //cout << "Transductive L2-SVM (TSVM)\n" << endl;
      if (verbose > 0)
        printf("Transductive L2-SVM (TSVM)\n");
      optimality=TSVM_MFN(Data,Options,Weights,Outputs, verbose);
      break;
    case DA_SVM:
      //cout << "Deterministic Annealing Semi-supervised L2-SVM (DAS3VM)\n" << endl;
      if (verbose > 0)
        printf("Deterministic Annealing Semi-supervised L2-SVM (DAS3VM)\n");
      optimality=DA_S3VM(Data,Options,Weights,Outputs, verbose);
      break;
    default:
      ;
    }
...
void ssl_predict_online(struct data *Data, const struct vector_double *Weights, struct vector_double *Outputs)
{
  double *w = Weights->vec;
  int n = Weights->d;
  int start_ix, stop_ix;
  double t=0.0;
  int i, j;
 /* read and predict */
  init_vec_double(Outputs,Data->m,0.0);
  ...
  {
    t = 0.0;
    start_ix = Data->rowptr[i];
    stop_ix = Data->rowptr[i+1];
    for (j = start_ix; j < stop_ix; ++j)
      t += Data->val[j] * w[Data->colind[j]];
    Outputs->vec[i] = t;
  }
}
/* Evaluation -- confusion matrix, accuracy, precision, recall, prbep */
/* roc area */
double ssl_evaluate(struct vector_double *Outputs, struct vector_double *TrueLabels, int verbose)
{ 
  double accuracy=0.0, mean_acc=0;
  for(int i=0;i<Outputs->d;i++)
    accuracy+=(Outputs->vec[i]*TrueLabels->vec[i])>0;
  //cout << "Accuracy = " << accuracy*100.0/Outputs->d << " %" << endl;
  mean_acc = accuracy*100.0/Outputs->d;
  if (verbose > 0)
    printf("Accuracy=%f\n", mean_acc);
...
{ delete[] c->vec; return;}
void clear_opt(struct options *opt)
{ delete[] opt; return;}
