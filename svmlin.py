'''  2018.7.13 Slightly modified by LingtingWu
...
ssl=imp.load_source('ssl','/home/wlt/anaconda3/python/py-svmlin/ssl.py')
import ssl
import ctypes
from ctypes import *
...
def ssl_transductive_train(Xl, Xn, Xu, verbose, **kwargs):
	...
	ssl_weights = ssl.vector_double()
	ssl_outputs = ssl.vector_double()
	ssl_options = ssl.options(**kwargs)

	ssl.libssl.ssl_train(ssl_data, ssl_options, ssl_weights, ssl_outputs, verbose)

	clf = np.array(np.fromiter(ssl_weights.vec, dtype=float, count=ssl_weights.d))

	#libssl.clear_data(ssl_data)
	ssl.libssl.clear_vec_double(ssl_outputs)
	ssl.libssl.clear_vec_double(ssl_weights)
...
def ssl_evaluate_online(X, y, w, verbose=True):
	...
	# C allocation
	ssl_weights =ssl. vector_double()
	ssl.libssl.init_vec_double(ssl_weights, len(w), 0.0)
	for i, v in enumerate(w):
		ssl_weights.vec[i] = v
	
	ssl_outputs = ssl.vector_double()
	
	ssl_gt = ssl.vector_double()
	ssl.libssl.init_vec_double(ssl_gt, len(y), 0.0)
	for i, v in enumerate(y):
		ssl_gt.vec[i] = v

	print ('')
	ssl.libssl.ssl_predict_online(ssl_data, ssl_weights, ssl_outputs)
	acc = ssl.libssl.ssl_evaluate(ssl_outputs, ssl_gt, verbose)

	# release memory
	ssl.libssl.clear_vec_double(ssl_weights)
	ssl.libssl.clear_vec_double(ssl_outputs)
	ssl.libssl.clear_vec_double(ssl_gt)
...
def ssl_evaluate(data_file, label_file, model_file, verbose=True):
	...
	print(ssl_outputs)
	ssl_gt = ssl.vector_double()
	ssl.libssl.init_vec_double(ssl_gt, len(gt), 0.0)
	for i, v in enumerate(gt):
		ssl_gt.vec[i] = v

       
	# test 
	ssl.libssl.ssl_predict(data_file, ssl_weights, ssl_outputs)
	# evaluate
	print ('11111')
	#acc = ssl.libssl.ssl_evaluate(ssl_outputs, ssl_gt, verbose)
	foo = ssl.libssl.ssl_evaluate
	foo.restype = c_double
	print('result:'+str(foo(ssl_outputs, ssl_gt, verbose)))
	#ssl.libssl.clear_vec_double(ssl_outputs)
	return str(foo(ssl_outputs, ssl_gt, verbose))


##################################################################
#print( 'Testing <ssl_train>...')
#w = ssl_train('/home/wlt/anaconda3/py-svmlin/example/training_data', True, algo=2, lambda_l=0.001, lambda_u=1)
#np.savetxt('/home/wlt/anaconda3/py-svmlin/example/weights', w)
#val = bytes('/home/wlt/anaconda3/py-svmlin/example/test_examples',encoding='utf-8')
#result_acc=ssl_evaluate(val, '/home/wlt/anaconda3/py-svmlin/example/test_labels', '/home/wlt/anaconda3/py-svmlin/example/weights', True)
#print(result_acc)
'''
print( '')
print ('Testing <ssl_transductive_train>...')
X, y = load_svmlight_file('/home/wlt/anaconda3/python/py-svmlin/example/training_data')
X = X.todense()
Xl = X[y==1.0,:]
Xn = X[y==-1.0,:]
Xu = X[y==0.0,:]
...
#np.savetxt('/home/wlt/anaconda3/py-svmlin/example/weights', w)
val = bytes('/home/wlt/anaconda3/python/py-svmlin/example/test_examples',encoding='utf-8')

result_acc=ssl_evaluate(val, '/home/wlt/anaconda3/python/py-svmlin/example/test_labels', '/home/wlt/anaconda3/python/py-svmlin/example/weights', True)
print(result_acc)
'''
